...
class Transaction(object):
    ...
    def __init__(self, date, descr0, account, descr1, descr2, descr3, amount, amount_f, valuta, exchangerate):
    ...
    def __repr__(self):
        return "[%d-%d-%d] %5.2f '%s'" % (self.date.day, self.date.month, self.date.year, self.amount, self.description)
    ...
    def to_qif(self):
        ...
        s += ["^"]
        s += ["!Type:Bank"]
        s += ["D%d/%d/%d" % (self.date.year, self.date.month, self.date.day)]
        s += ["T%.2f" % self.amount]
        s += ["NTransfer"]
